...
**预期输出：**
```
状态: { task: "计算 23 * 45", observations: [], actions: [] }
→ LLM 思考：需要计算 23 * 45
→ 行动：调用计算器工具
→ 观察：23 * 45 = 1035
→ 任务完成，返回结果: 1035
...
### 技巧一：利用 System Prompt 定义 Agent 角色

Agent 的行为高度依赖于 System Prompt。一个好的 System Prompt 应该明确 Agent 的「人格」和「能力边界」。
...
你的约束：
- 不要执行可能造成破坏的命令（rm -rf、DROP TABLE 等）
- 每次修改前先读取目标文件
- 不确定时向用户确认
...
在多步任务中，Agent 应该优先选择「低成本」的工具。通过给工具添加成本元数据，可以引导 Agent 更高效地规划。

```typescript
interface ToolWithCost {
  name: string;
  cost: 'low' | 'medium' | 'high';  // 执行成本
...
确保 Agent 的每次行动都有明确的成功/失败信号返回，避免 Agent 「猜测」结果。
...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 认为所有 LLM 都能直接当 Agent | LLM 仅提供推理能力，缺少工具、记忆和规划循环 | Agent = LLM + 工具 + 记忆 + 规划，缺一不可 |
| 把 Chatbot 的对话轮次当 Agent 能力 | 多轮对话 ≠ Agent 自主行动 | 检查 Agent 是否有工具调用和外部系统交互能力 |
| 忽略错误处理导致 Agent 卡死 | Agent 循环中没有处理工具调用失败的情况 | 每次工具调用都要捕获异常，给 LLM 明确的错误反馈 |
