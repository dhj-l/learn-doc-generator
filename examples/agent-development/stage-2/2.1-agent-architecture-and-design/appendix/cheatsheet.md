...
| 架构 | 核心思想 | 适用场景 | 核心实现 |
|------|----------|----------|----------|
| **ReAct** | 推理+行动交替 | 通用任务 | `while(!done) { thought → action → observe }` |
| **Plan-and-Execute** | 先规划后执行 | 复杂多步任务 | `plan = planner(task); plan.forEach(executor.step)` |
| **Reflexion** | 尝试→评估→反思→重试 | 迭代改进 | `for(i=0;i<max;i++){ result=try(task); self_eval(result) }` |
