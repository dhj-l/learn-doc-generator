...
### 技巧一：使用 Zod Schema 自动生成 TypeScript 类型

`zodResponseFormat` 最强大的特性是——你定义一次 Schema，就能同时获得运行时验证和 TypeScript 类型推断：

```typescript
// 定义 Schema → 自动得到类型 + 运行时验证 + API 格式
...
// 一行代码传给 OpenAI
const format = zodResponseFormat(UserSchema, 'user');
// parse() 返回的就是类型安全的 User 对象
const user: User = response.choices[0].message.parsed!;
```

### 技巧二：嵌套 Schema 处理复杂数据结构
...
**Q1：JSON Mode 和 Structured Outputs 的区别是什么？**

> A：JSON Mode 只保证输出「有效的 JSON」，但结构可能变化——可能返回 `{name:"张三"}` 也可能返回 `{user:{name:"张三"}}`。Structured Outputs 严格遵循你定义的 Schema，输出结构完全可预测。如果你的下游代码要 `JSON.parse` 后直接访问字段，用 Structured Outputs。

**Q2：为什么 Zod Schema 比直接用 JSON Schema 更推荐？**

> A：Zod 提供「单一真相源」——Schema 同时用于 TypeScript 类型推断、运行时验证、OpenAI 的 response_format。改动 Schema 时，类型自动更新、验证自动同步。直接用 JSON Schema 需要手动维护类型定义和 API 格式的一致性。

**Q3：Structured Outputs 的 `additionalProperties: false` 有什么作用？**

> A：它告诉模型「只输出 Schema 中定义的字段，不要加额外字段」。如果不设置，模型可能会在 JSON 中添加 Schema 之外的字段，导致下游代码（特别是 Zod 验证）失败。设置 `false` 强制输出严格匹配 Schema。
...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| JSON Mode 输出不是合法 JSON | 模型输出的 JSON 中包含注释或多余文本 | 在 Prompt 中明确「只输出 JSON，不要加任何解释」 |
| Structured Outputs 提示模型不支持 | 使用了过旧的模型 | 使用 `gpt-4o-2024-08-06` 或更新的模型 |
| Zod Schema 验证不通过 | 模型输出的字段值超出了 Schema 约束 | 在 Zod 中使用 `.describe()` 提供更清晰的约束说明 |
| 嵌套对象的某些字段始终为 null | 模型不理解嵌套结构的深度 | 减少嵌套深度（最多 2-3 层），用 `optional()` 标记非必需字段 |
| JSON 中的枚举值不匹配 | 枚举名称使用了模型的非标准说法 | 在 `z.enum()` 的描述中提供明确的枚举值含义解释 |
