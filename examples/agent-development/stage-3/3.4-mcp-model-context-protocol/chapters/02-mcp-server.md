...
### 技巧一：Tool 参数的 Schema 设计原则

- 使用 `z.string().describe()` 提供清晰的参数说明，帮助 LLM 理解如何传参
- 优先使用简单类型（string, number, boolean），避免过于复杂的嵌套
- 用 `.default()` 提供合理的默认值，减少 LLM 需要做的决策
- 用 `.optional()` 标记可选参数，减少 LLM 的调用负担

### 技巧二：错误处理的三种模式

```typescript
// 模式 1：业务错误（返回给 LLM，让 Agent 决定下一步）
return { content: [{ type: 'text', text: '文件不存在' }], isError: true };

// 模式 2：可预期错误（友好提示 + 建议）
return { content: [{ type: 'text', text: `无法读取 ${path}，请检查路径是否正确` }], isError: true };
...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| Server 启动后立即退出 | 没有 await server.connect(transport) | 确保在 main 函数中调用了 await server.connect(transport) |
| 工具调用返回 "Tool not found" | 工具名不匹配或带了 namespace | 使用 listTools() 返回的原始工具名调用 |
| Resource URI 格式错误 | URI 不符合标准格式 | 确保 URI 使用 scheme://path 的格式 |
| 参数验证失败不返回错误信息 | isError: true 但没有说明 | 在返回的 text 中包含具体的错误原因 |
...
> [第3章：TypeScript SDK 深入](./03-typescript-sdk.md) — 认证、错误处理与进度报告。
