...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| generateText 返回内容被截断 | maxTokens 设置过小 | 根据实际需要设置合适的 maxTokens 值 |
| streamText 在前端无法正确解析 | Data Stream 协议实现不完整 | 使用 ai 包提供的 readDataStream 工具函数解析 |
| 多模型切换时报 Provider 错误 | 环境变量未正确设置 | 检查每个 Provider 对应的 API Key 环境变量 |
| generateObject 输出不符合 Schema | Zod 定义与实际 LLM 输出不匹配 | 使用 .partial() 允许部分匹配，添加重试逻辑 |
