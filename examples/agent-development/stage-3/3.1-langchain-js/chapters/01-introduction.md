...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 模型 API 调用超时 | 网络延迟或 API 限流 | 添加重试机制，使用 .withFallbacks() 降级 |
| Prompt 模板渲染失败 | 模板变量名与传入参数不匹配 | 检查模板中的 {变量} 名称与 invoke 参数一致 |
| Chain 输出类型不符合预期 | pipe 串联时类型不兼容 | 使用 StringOutputParser 确保输出为字符串 |
