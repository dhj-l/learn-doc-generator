...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 在 LCEL 管道中传递了不匹配的输入/输出类型 | 前一个组件的输出类型与下一个组件的输入类型不兼容 | 使用 `RunnablePassthrough` 转换数据，或检查组件的泛型类型签名 |
| 误用 `RunnableParallel` 导致变量未在后续步骤中展开 | `RunnableParallel` 返回的对象需要正确解构 | 使用 `.pipe()` 或将并行结果通过 `RunnablePassthrough` 传递到下一节点 |
| 条件分支函数未正确处理所有输入情形 | `ConditionalEdge` 的路由函数只覆盖了部分 case | 确保路由函数包含兜底逻辑（default branch），返回一个有效的节点名称 |
| 调用 `.stream()` 时未正确消费异步迭代器 | 直接 `for...of` 而不是 `for await...of` | 使用 `for await (const chunk of chain.stream(input))` 消费流式数据 |
