...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 混淆 `@langchain/core` 与 `@langchain/community` 等扩展包 | 不清楚每个包的定位和职责 | 核心接口在 `@langchain/core`，模型实现如 `@langchain/anthropic` 是扩展包，按需安装 |
| 调用模型时忘记传 API Key 或 Key 配置错误 | 未正确设置环境变量或直接硬编码 | 使用 `process.env.ANTHROPIC_API_KEY` 读取环境变量，不要硬编码 |
| 认为 LangChain 必须搭配某个特定 LLM | 误解 LangChain 是模型而非框架 | LangChain 是统一调用框架，可自由切换 Claude、GPT 等模型 |
| 忽略异步环境，在顶层使用 `await` 报错 | Node.js 顶层不支持 `await` | 将代码包裹在 `async function main() { ... }` 中并调用 `main().catch(console.error)` |
