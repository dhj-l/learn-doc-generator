...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 注册 tool 时参数 Schema 过于简单导致 LLM 传入错误参数 | 未使用 Zod Schema 做参数校验，或 Schema 缺少必要描述 | 为每个工具参数使用 `z.object()` 定义完整的 Schema，并添加详细的 description |
| 资源 URI 设计不合理导致客户端无法正确访问 | URI 路径中缺少版本号或包含特殊字符 | 遵循 RESTful URI 设计规范，使用 `resource://` 前缀 + 清晰路径，避免特殊字符 |
| Server 启动后客户端连接失败 | 传输协议不匹配（Server 用 stdio，Client 用 HTTP） | 确保 Server 和 Client 使用相同的传输协议，双方配置一致 |
| 工具执行中抛出异常未处理导致 Server 崩溃 | 工具函数未使用 try-catch 捕获内部错误 | 在工具函数中使用 try-catch 捕获所有异常，通过 `McpError` 返回标准错误响应 |
