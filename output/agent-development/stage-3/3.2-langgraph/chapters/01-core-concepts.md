...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| Node 函数返回了不存在的 state 字段 | 状态 Schema 中未声明该字段，导致合并时报错 | 在 `Annotation.Root` 中先定义所有字段，或使用 `add` reducer 动态添加 |
| 条件边路由函数返回的节点名称在图中不存在 | 函数逻辑返回了拼写错误或未注册的节点名 | 确保路由函数的返回值与 `graph.addNode(...)` 注册的名称完全一致 |
| 忘记在 State 中使用 reducer 导致旧值被覆盖 | 默认的 reducer 是覆写模式，不会追加数组元素 | 数组类型的字段使用 `Annotation({ reducer: (a, b) => a.concat(b) })` |
| 检查点未生效，图执行状态无法恢复 | 未配置 `Checkpointer` 或配置了错误的持久化后端 | 创建 StateGraph 时需要传入 `checkpointer` 参数，如 `new StateGraph({ checkpointer: new MemorySaver() })` |
