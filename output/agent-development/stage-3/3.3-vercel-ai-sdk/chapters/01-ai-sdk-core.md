...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| `generateObject` 返回的数据结构与 Zod Schema 不匹配 | LLM 输出不符合预期的 JSON 格式或 Schema 约束 | 在 `generateObject` 的 schema 参数中使用 `z.object` 明确定义字段，并启用 `mode: 'json'` 强制模型输出 JSON |
| 工具调用时未设置 `maxSteps` 导致只能执行一轮工具 | 默认 `maxSteps` 为 1，工具返回后直接结束 | 显式设置 `maxSteps`（如 5~10），让 Agent 能根据工具返回结果继续推理 |
| `streamText` 的 `textStream` 与 `fullStream` 混淆 | 不清楚两者的区别：`textStream` 只输出文本，`fullStream` 包含完整事件 | 只需纯文本用 `textStream`，需要工具调用等事件信息用 `fullStream` |
| 跨模型调用时参数不兼容 | 不同模型 Provider 的模型 ID 或参数名不一致 | 使用统一 Provider 接口，模型 ID 遵循 Provider 文档中的命名规范 |
